    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        # Choose the most likely ghost
        ghost_choice = []
        for ghost_dist in livingGhostPositionDistributions:
          max_key = ghost_dist.argMax()
          ghost_choice.append(max_key)
        
        # Choose the nearest ghost
        min_dist = float("inf")
        ghost_chosen = None
        for ghost_pos in ghost_choice:
          dist = self.distancer.getDistance(ghost_pos,pacmanPosition)
          if (dist < min_dist):
            min_dist = dist
            ghost_chosen = ghost_pos
        ...
        for action in legal:
          ...
          dist = self.distancer.getDistance(successor,ghost_chosen)
          if (dist < min_dist):
            min_dist = dist
            chosen_act = action
