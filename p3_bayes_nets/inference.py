...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for var in eliminationOrder:
            current_factors, joinedFactor = joinFactorsByVariable(current_factors, var)
            if len(joinedFactor.unconditionedVariables()) != 1:
                eliminated = eliminate(joinedFactor, var)
                current_factors.append(eliminated)

        joined_factor = joinFactors(current_factors)
        normalizedd_factor = normalize(joined_factor)

        return normalizedd_factor
