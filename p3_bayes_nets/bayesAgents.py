...
def constructBayesNet(gameState):
    ...
    "*** YOUR CODE HERE ***"
    for p_var in [X_POS_VAR, Y_POS_VAR]:
        for house_var in HOUSE_VARS:
            edges.append((p_var,house_var))
...
def fillObsCPT(bayesNet, gameState):
    ...
    house_pos = gameState.getPossibleHouses()
    house_pos_val = [BOTTOM_LEFT_VAL, TOP_LEFT_VAL,BOTTOM_RIGHT_VAL, TOP_RIGHT_VAL]

    def check(wall, house, house_v):
        pos = None
        for i in range(4):
            x_dis = abs(wall[0] - house[i][0])
            y_dis = abs(wall[1] - house[i][1])
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    query_var = [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR]
    factor = inference.inferenceByVariableElimination(bayesNet, query_var, evidence,eliminationOrder)
    prob = 0.0
    max_assignment = {}

    for assignment in factor.getAllPossibleAssignmentDicts():
        cur_prob = factor.getProbability(assignment)
        if (cur_prob > prob):
            prob = cur_prob
